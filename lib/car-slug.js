...
function duplicatesOf(car, cars) {
  if (!Array.isArray(cars) || !cars.length) return [car];
  const mine = groupKey(car);
  const group = cars.filter((c) => groupKey(c) === mine);
  group.sort((a, b) => {
    const ta = Date.parse(a.created_at || "") || 0;
    const tb = Date.parse(b.created_at || "") || 0;
    if (ta !== tb) return ta - tb;
...
/**
 * `overrides` maps a group key to the car id the owner picked, so the choice
 * survives deleting whichever car happened to be added first. A stale override,
 * pointing at a car that no longer exists, simply falls back to the default.
 */
function primaryFor(car, cars, overrides) {
  const group = duplicatesOf(car, cars);
  if (group.length < 2) return car;
  const chosen = overrides && overrides[groupKey(car)];
  if (chosen != null) {
    const hit = group.find((c) => String(c.id) === String(chosen));
    if (hit) return hit;
  }
  return group[0];
}

function isPrimary(car, cars, overrides) {
  return String(primaryFor(car, cars, overrides).id) === String(car.id);
}
...
function matchCar(slug, cars) {
  ...
  const exact = cars.find((c) => baseSlug(c) === wanted);
  if (exact) return exact;
  // a car renamed in the dashboard ("bmw-g30-2020" became "bmw-seria-5-g30-2020"):
  // every word of the old address is still in exactly one car's address
  const words = wanted.split("-").filter(Boolean);
  if (words.length < 2) return null;
  const hits = cars.filter((c) => {
    const have = new Set(String(m.get(String(c.id)) || baseSlug(c)).split("-"));
