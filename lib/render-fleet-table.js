...
const carWords = require("./car-words");
const { isDriverOnly, NOTE: DRIVER_NOTE } = require("./driver-only");

const T = {
  ro: {
    heading: "Flota și tarifele pe zi",
    intro: "Tarifele scad cu numărul de zile. RCA și 200 km pe zi sunt incluse, fără avans.",
    more: "Tabelul arată mașinile libere acum. Flota are peste 70 de vehicule, așa că dacă nu vezi modelul căutat, sună la +373 60 000 500 și îți spunem ce este disponibil pe datele tale.",
  ...
  ru: {
    ...
    more: "В таблице машины, свободные сейчас. Всего в автопарке более 70 автомобилей, поэтому если нужной модели нет в списке, позвоните на +373 60 000 500, подскажем, что свободно на ваши даты.",
...
function word(group, value, lang) {
  if (!value) return "-";
  return carWords.word(group, value, lang);
}
...
function table(cars, lang) {
  ...
  const rows = cars.map((car) => {
    ...
    return `
        <tr>
          <th scope="row"><a href="${pathFor(car, lang, cars)}">${esc(name)}</a>${isDriverOnly(car) ? ` <span class="pr-fleet-driver">${esc(DRIVER_NOTE[lang] || DRIVER_NOTE.ro)}</span>` : ""}${car.booked ? ` <span class="pr-fleet-busy">${esc(t.unavailable)}</span>` : ""}</th>
  ...
  const items = cars.map((car, i) => {
    ...
    const node = {
      ...
      item: {
        ...
        // the same entity the car's own page describes, not a second copy of it
        "@id": "https://plusrent.md" + pathFor(car, lang, cars) + "#car",
        name,
