...
const { businessNodes } = require("./business-node");
const { word, engine } = require("./car-words");
const { isDriverOnly, NOTICE: DRIVER_NOTICE } = require("./driver-only");

// "Rent a Audi" reads wrong; the article follows the sound of the make.
/** The brand goes at the end of a title only while the whole title stays within 60 characters. */
function withBrand(title) {
  const full = `${title} | PlusRent`;
...
const T = {
  ro: {
    htmlLang: "ro",
    ogLocale: "ro_RO",
    title: (n, y) => withBrand(`Chirie ${n}${y ? " " + y : ""} în Chișinău: preț pe zi`),
    desc: (n, price, seats, gear, driverOnly) => driverOnly
      ? `${n} în Chișinău, doar cu șofer personal PlusRent, de la €${price} pe zi. ${seats} locuri, cutie ${gear}, RCA și 200 km pe zi incluse, fără avans.`
      : `Închiriază ${n} în Chișinău de la €${price} pe zi. ${seats} locuri, cutie ${gear}, RCA și 200 km pe zi incluse, fără avans. Livrare la aeroport.`,
    ...
    airport: { before: "Mașina poate fi predată și la aeroport: ", link: "chirie auto la Aeroportul Chișinău", href: "/ro/chirie-auto-aeroport-chisinau" },
    ...
    mega: ["Prețuri și flotă", "Șofer Treaz", "Transfer Aeroport KIV", "Transfer Aeroport IAS", "Șofer Personal"],
  },
  ru: {
    htmlLang: "ru",
    ogLocale: "ru_RU",
    title: (n, y) => withBrand(`Аренда ${n}${y ? " " + y : ""} в Кишиневе: цена за сутки`),
    desc: (n, price, seats, gear, driverOnly) => driverOnly
      ? `${n} в Кишиневе только с личным водителем PlusRent, от €${price} в сутки. ${seats} мест, коробка ${gear}, RCA и 200 км в сутки включены, без предоплаты.`
      : `Возьмите ${n} в аренду в Кишиневе от €${price} в сутки. ${seats} мест, коробка ${gear}, RCA и 200 км в сутки включены, без предоплаты. Доставка в аэропорт.`,
    h: "Об этой машине",
    ...
    airport: { before: "Машину можно получить и в аэропорту: ", link: "аренда авто в аэропорту Кишинёва", href: "/ru/arenda-avto-aeroport-kishinev" },
    ...
    calculating: "Считаем цену...",
    nav: { home: "Главная", cars: "Автомобили", about: "О нас", contact: "Контакты", services: "Услуги", top: "Наверх" },
    mega: ["Цены и автопарк", "Трезвый водитель", "Трансфер аэропорт Кишинев", "Трансфер аэропорт Яссы", "Личный водитель"],
  },
  en: {
    htmlLang: "en",
    ogLocale: "en_US",
    title: (n, y) => withBrand(`Rent ${article(n)} ${n}${y ? " " + y : ""} in Chisinau: price per day`),
    desc: (n, price, seats, gear, driverOnly) => driverOnly
      ? `${n} in Chisinau, only with a PlusRent driver, from €${price} a day. ${seats} seats, ${gear} gearbox, insurance and 200 km/day included, no prepayment.`
      : `Rent ${article(n)} ${n} in Chisinau from €${price} a day. ${seats} seats, ${gear} gearbox, insurance and 200 km a day included, no prepayment. Airport delivery.`,
    h: "About this car",
    specs: "Specification",
    ...
    airport: { before: "You can also collect it at the airport: ", link: "car rental at Chisinau Airport", href: "/en/car-rental-chisinau-airport" },
    ...
    mega: ["Prices &amp; fleet", "Sober Driver", "Airport Transfer Chisinau", "Airport Transfer Iasi", "Personal Driver"],
  },
};
...
/** Kept as a name this file already used; the rule itself is shared now. */
...
function facts(car, lang) {
  ...
  const rows = [
    ...
    [t.type, word("type", car.car_type, lang)],
    [t.gear, gearWord(car, t)],
    [t.fuel, word("fuel", car.fuel_type, lang)],
    [t.seats, car.num_passengers],
    [t.doors, car.num_doors],
    [t.drive, car.drive],
    [t.luggage, car.luggage],
    [t.engine, engine(car.engine_capacity, lang)],
      ...
      <section class="pr-car-facts" aria-labelledby="pr-car-facts-h">
        ...
        <div class="pr-car-facts-grid">
          ...
          <div>
            ...
            <p class="pr-car-airport">${esc(t.airport.before)}<a href="${t.airport.href}">${esc(t.airport.link)}</a>.</p>
          </div>
        </div>
      </section>`;
}
...
function jsonLd(car, lang, cars) {
  ...
  const properties = [
    ...
    spec(t.engine, engine(car.engine_capacity, lang)),
  ...
  return {
    ...
    "@graph": [
      vehicle,
      // The seller and the site this page names by @id, defined here so the
      // references resolve on this page instead of pointing at nothing.
      ...businessNodes(lang),
      {
        "@type": "BreadcrumbList",
        "@id": url + "#breadcrumb",
...
function localiseLabels(html, lang) {
  ...
  // the price box's loading line, which a crawler reads as it stands
  html = html.split(">Calculating price...</span>").join(`>${t.calculating}</span>`);
  return html;
}
...
/**
 * The first photo, in the HTML rather than added by a script.
 *
 * The gallery was empty until JavaScript filled it, so a crawler that does not
 * run scripts saw a car page with no picture of the car, and the browser could
 * not start downloading the largest image until the script had run. The URL and
 * srcset here are the ones car-image.js builds, so the script adopts this image
 * instead of downloading it a second time.
 */
const SUPABASE_IMAGES = "https://ncmqbrhlxjrjgnkaafhv.supabase.co/storage/v1/object/public/car-images/";

function firstPhoto(car) {
  let gallery = car.gallery_images;
  if (typeof gallery === "string") { try { gallery = JSON.parse(gallery); } catch { gallery = null; } }
  const first = Array.isArray(gallery) && gallery.length ? gallery[0] : car.head_image;
  if (!first) return null;
  return String(first).startsWith("http") ? String(first) : ORIGIN + first;
}

function heroImage(car, name) {
  const src = firstPhoto(car);
  if (!src || src.indexOf(SUPABASE_IMAGES) !== 0) return "";
  const opt = (w) => `/_vercel/image?url=${encodeURIComponent(src)}&w=${w}&q=90`;
  const srcset = [640, 828, 1080, 1920].map((w) => `${opt(w)} ${w}w`).join(", ");
  const alt = `${name}${car.production_year ? " " + car.production_year : ""}`;
  return `<img data-ssr-photo src="${esc(opt(1080))}" srcset="${esc(srcset)}" sizes="(max-width: 991px) 100vw, 900px" width="1080" height="810" alt="${esc(alt)}" fetchpriority="high" decoding="async" class="active pr-img-loaded">`;
}

const MOBILE_BAR = {
  ro: { from: "de la", unit: "/zi", cta: "Rezervă acum" },
  ru: { from: "от", unit: "/сутки", cta: "Забронировать" },
  en: { from: "from", unit: "/day", cta: "Book now" },
};
...
function render(template, car, lang, cars, overrides) {
  ...
  html = html.replace("<!-- Main images will be inserted here -->", heroImage(car, name));
  // the booking bar pinned to the bottom on phones, in the page's language
  const bar = MOBILE_BAR[lang] || MOBILE_BAR.ro;
  html = html.replace(
    /<div class="pr-mb-price"><small>de la<\/small><b><span id="pr-mb-price">€15<\/span> <span class="pr-mb-unit">\/zi<\/span><\/b><\/div>\s*<a href="#contact_form" class="pr-mb-btn">Rezervă acum<\/a>/,
    `<div class="pr-mb-price"><small>${bar.from}</small><b><span id="pr-mb-price">€${low === null ? "15" : low}</span> <span class="pr-mb-unit">${bar.unit}</span></b></div>
      <a href="#contact_form" class="pr-mb-btn">${bar.cta}</a>`
  );

  // the sidebar heading too, which was empty until the script filled it
  html = html.replace(/(<h2 id="car-name"[^>]*>)(<\/h2>)/i, `$1${esc(name)}$2`);
  // the template's €60 is what crawlers read on every car until the script runs
  html = html.replace(
    /(<div class="price-amount" id="car-price-sidebar">)[^<]*(<\/div>)/i,
    `$1€${low === null ? "15" : low}$2`
  );
  if (isDriverOnly(car)) {
    html = html.replace(
      /(<h2 id="car-name"[^>]*>[^<]*<\/h2>)/i,
      `$1
                  <p class="pr-driver-only" role="note">${esc(DRIVER_NOTICE[lang] || DRIVER_NOTICE.ro)}</p>`
    );
  }
