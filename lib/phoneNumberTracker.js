...
async function trackPhoneNumberForBooking(phoneNumber, bookingId) {
  try {
    console.log(`📞 Tracking phone number: ${phoneNumber} for booking: ${bookingId}`);
    ...
    console.log(`📞 Normalized phone number: ${phoneNumber} → ${normalizedPhoneNumber}`);
    ...
    if (data && data.length > 0) {
      ...
      if (!existingRow.bookings_ids.includes(bookingId)) {
        ...
        console.log(`✅ Successfully updated phone number ${normalizedPhoneNumber} with booking ${bookingId}`);
        return {
          success: true,
          data: updateData[0],
        ...
        console.log(`ℹ️  Booking ${bookingId} already tracked for phone number ${normalizedPhoneNumber}`);
        return {
          success: true,
          data: existingRow,
    ...
    console.log(`✅ Successfully created new phone number entry for ${normalizedPhoneNumber} with booking ${bookingId}`);
    return {
      success: true,
      data: data[0],
...
async function trackPhoneNumberForSpinningWheel(phoneNumber) {
  try {
    console.log(`📞 Tracking phone number for spinning wheel: ${phoneNumber}`);
    ...
    if (existingData) {
      console.log(`ℹ️  Phone number ${normalizedPhoneNumber} already exists in tracking`);
      return {
        success: true,
        data: existingData,
    ...
    console.log(`✅ Successfully created new phone number entry for ${normalizedPhoneNumber}`);
    return {
      success: true,
      data: data,
      message: 'Phone number tracking created successfully',
